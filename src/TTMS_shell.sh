...
if [ "$?" -eq "1" ]; then
    ...
    echo "Studio.dat updated ！\n"
    flag=$((${flag}+1))
fi

if [ "${flag}" -eq "0" ];then
    echo "${date_tmp}  File not updated ！\n" >> systemlog
    temp=1
    flag=$((${flag}+1))
fi
