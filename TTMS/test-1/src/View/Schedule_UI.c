...
void Shedule_UI_MgtEntry(int play_id)
{
	int i,id;
	char choice;
	...
	paging.totalRecords = Play_Srv_FetchByPlay(head,play_id);
	...
	do
	{
		...
		printf( "%20s%20s%20s%20s%20s","shang ying ju mu","yan chu ding id","fang ying ri qi","fang ying shi jian","seat_count\n");
		printf( "---------------------------------------------------------------\n");
		Paging_ViewPage_ForEach(head,paging,schedule_node_t,pos,i)
		{
			printf("%3d %3d %3d %3d %3d %3d %3d %3d %3d %3d",pos->data.play_id,pos->data.studio_id,pos->data.date.year,pos->data.date.month,pos->data.date.day,pos->data.time.hour,pos->data.time.minute,pos->data.time.second,pos->data.seat_count);
		}
